...
#                                      est <- fixest_mod_select(model = model, fmla = fixest_fmla, base = datab4(), famly = fmly)
#
#                                      stats_fmla <- xpd(lhs ~ x + as.factor(grp) + as.factor(tm), lhs = y_dep)
#                                      est_bis <- stats_mod_select(model = model, fmla = stats_fmla, base = datab4(), famly = fmly)
#
...
#                                        vcov_sw <- sqrt(vcovCL(est_bis, cluster = datab4()$grp, type = "HC0")["x", "x"])
#                                        expect_equal2(vcov_fxt, vcov_sw, tol = 1e-6) # Almost equal
#
#                                        # Clusteres SE type HC1
#                                        vcov_fxt <- unname(se(est, dof = dof(fixef.K = "full")))
#                                        vcov_sw <- sqrt(vcovCL(est_bis, cluster = datab4()$grp, type = "HC1")["x", "x"])
#                                        expect_equal2(vcov_fxt, vcov_sw, tol = 1e-6) # Almost equal
#                                      })
#
...
#   expect_error(se(est_pois, cluster = base$Origin_na)) ## No arroja error!!
