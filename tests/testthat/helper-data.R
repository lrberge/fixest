...
ols_cases = function(){
    ...
    for(weights in c("NULL", "base$w")){
        for(offset in c("NULL", "base$offset_value")){
            DF_l[[cont]] = data.frame(fml_fixest = fml_fixest,
                                      fml_stats = fml_stats,
                                      my_offset = offset,
                                      my_weight = weights)
            cont = cont + 1
...
feglm_cases = function(Models = c("binomial", "poisson", "Gamma")){
    ...
    for(model in Models){
        ...
        my_fam = switch(model, binomial = "binomial", poisson = "poisson", negbin = "NULL", Gamma = "Gamma")
...
fenegbin_cases = function(){
    fml_fixest1 = c("y_int ~ x1",
                    "y_int ~ x1 | species"
    )
    fml_stats1 = c("y_int ~ x1",
                   "y_int ~x1 + factor(species)"
    )
    DF = data.frame(fml_fixest = fml_fixest1,
...
vcov_cases1 = function(){
    DF = data.frame(expand.grid(c(FALSE, TRUE),
                                c("none", "nested", "full")))
...
vcov_cases2 = function(){
    DF = data.frame(expand.grid(c("none", "nested", "full"),
                                c("conventional", "min"),
                                c(FALSE, TRUE),
                                c(FALSE, TRUE)))
...
vcov_cases3 = function(){
    DF = data.frame(expand.grid(c("conventional", "min"),
                                c("conventional", "min"),
                                c("none", "nested", "full"),
                                c(FALSE, TRUE),
                                c(FALSE, TRUE)))
