...
# ~: package sniff tests
# Modified 2021-11-22 Mauricio Vargas + Hernan Robledo
#----------------------------------------------#
...
for (useWeights in c(FALSE, TRUE)) {
    for (model in c("ols", "pois", "glm", "negbin", "femlm", "feNmlm")) {
        for (use_fe in c(FALSE, TRUE)) {
            ...
            if (model == "ols") {
                ...
                if (!use_fe) {
                    ...
                    if (model == "glm") {
                        for (fmly in c("gaussian", "inverse.gaussian", "quasi")) {
                            res <- feglm(y_int ~ x1 + constant, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int ~ x1 + constant, base, weights = my_weight, family = fmly)
                        }
                        for (fmly in c("Gamma")) {
                            res <- feglm(y_int ~ x1 + constant, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int ~ x1 + constant, base, weights = my_weight, family = fmly)
                        }
                        for (fmly in c("poisson", "quasipoisson")) {
                            res <- feglm(y_int_null ~ x1 + constant, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int_null ~ x1 + constant, base, weights = my_weight, family = fmly)
                        }
                    }
                } else {
                    if (model == "pois") {
                    ...
                    if (model == "glm") {
                        for (fmly in c("gaussian", "inverse.gaussian", "quasi")) {
                            res <- feglm(y_int ~ x1 + constant | species, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int ~ x1 + constant + species, base, weights = my_weight, family = fmly)
                        }
                        for (fmly in c("Gamma")) {
                            res <- feglm(y_int ~ x1 + constant | species, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int ~ x1 + constant + species, base, weights = my_weight, family = fmly)
                        ...
                        for (fmly in c("poisson", "quasipoisson")) {
                            res <- feglm(y_int_null ~ x1 + constant | species, base, weights = my_weight, family = fmly)
                            res_bis <- glm(y_int_null ~ x1 + constant + species, base, weights = my_weight, family = fmly)
                        }
                    }
                }
                adj <- 0
...
test(
  predict(a, se.fit = TRUE, interval = "pre")$fit[, 2],
...
test(
  predict(aw, se.fit = TRUE, interval = "pre")$fit[, 2],
