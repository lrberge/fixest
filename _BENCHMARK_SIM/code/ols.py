    ...
    for dgp in ("simple", "difficult"):
      ...
      for n_fe in (2, 3):
        ...
        if dgp == "simple":
          ...
          # we don't run for large number of obs. b/c no convergence or run time > 1h
          if n_obs >= 1e6: 
            continue
        ...
        est = pyfixest.feols(fml, base)
        time_feols = time.perf_counter() - time_start
        ...
        result = pandas.DataFrame({
          ...
          'method': ["pyfixest.feols"], 'time': [time_feols]
        })
...
all_results.to_csv("./results/results_ols_py.csv", index = False)
