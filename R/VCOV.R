...
vcov.fixest = function(object, vcov = NULL, se = NULL, cluster, ssc = NULL, attr = FALSE, 
  ...
  # Note: vcov_hetero_internal no longer applies adjustment; alternatively do not apply here and check for `hc1` in vcov_hetero_internal
...
vcov_hetero_internal = function(bread, scores, sandwich, ssc, nthreads, ...){
  # N/(N-1) Adjustment occurs in `vcov` now to account for hc2/hc3
  if(!sandwich){
...
oldargs_to_vcov = function(se, cluster, vcov, .vcov = NULL){
  ...
  if(!missnull(.vcov)){
    ...
    if (is.list(.vcov)) {
      check_value(.vcov[[1]], "matrix", .arg_name = ".vcov")
    }
