...
est_feols = feols(y ~ x | id + time, base, panel.id = ~id + time)
...
rbind(se(est_lm)["x"], se(est_plm)["x"], se(est_feols, vcov = "iid"))

# p-values:
rbind(pvalue(est_lm)["x"], pvalue(est_plm)["x"], pvalue(est_feols, vcov = "iid"))
...
```{r}
# Clustered by id
se_lm_id    = se(vcovCL(est_lm, cluster = base$id, type = "HC1"))["x"]
se_plm_id   = se(vcovHC(est_plm, cluster = "group"))["x"]
...
se_feols_id_lm = se(est_feols, ssc = ssc(fixef.K = "full"))
rbind(se_lm_id, se_feols_id_lm)

# How to get the plm version
se_feols_id_plm = se(est_feols, ssc = ssc(fixef.K = "none", cluster.adj = FALSE))
...
se_lm_NW    = se(vcovPL(est_lm, cluster = ~ id + time))["x"]
se_plm_NW   = se(vcovNW(est_plm))["x"]
se_feols_NW = se(est_feols, vcov = "NW")

rbind(se_lm_NW, se_plm_NW, se_feols_NW)

# we can replicate plm's by changing the type of SSC:
rbind(se_plm_NW, 
      se(est_feols, vcov = NW ~ ssc(adj = FALSE, cluster.adj = FALSE)))

# Note that sandwich::vcovPL could not be replicated

#
# Driscoll-Kraay
#

se_plm_DK   = se(vcovSCC(est_plm))["x"]
