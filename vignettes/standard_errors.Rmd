...
In `fixest`, the `summary` method is used to obtain coefficient tables with different types of standard-errors. Here's an example, the explanations follow in the next two sections:
```{r}
library(fixest)
data(trade)
# OLS estimation
...
rbind(se(res_lm), se(res_feols, se = "standard"))

# Heteroskedasticity-robust covariance
se_lm_hc    = sqrt(diag(vcovHC(res_lm, type = "HC1")))
se_feols_hc = se(res_feols) # by default, it's the HC1 VCOV
...
## Changelog
 ...
 - Version 0.8.0:
    ...
    * The new default standard-errors for estimations without fixed-effects is heteroskedasticity-robust SEs (i.e. `se = "hetero"`).
