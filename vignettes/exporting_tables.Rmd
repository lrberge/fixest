...
This document does not describe `etable`'s arguments in details (the help page provides many examples). Rather, it illustrates some powerful features that may be hidden at first sight.
...
setFixest_dict(c(Ozone = "Ozone (ppb)", Solar.R = "Solar Radiation",
...
Thanks to contributors (namely Karl Dunkle Werner), `fixest` objects are compatible with [broom](https://broom.tidymodels.org/index.html) methods. This means that export functions building on `broom` can be leveraged, like for instance [modelsummary](https://github.com/vincentarelbundock/modelsummary).
...
The previous code returns a list of the five estimations for which the standard-errors are all clustered at the Month level.
