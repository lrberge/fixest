...
Using data from the airquality data base, let's estimate 4 models:

```{r}
library(fixest)
data(airquality)

setFixest_notes(FALSE) # Avoid messages when NAs are removed
...
```{r, eval = FALSE}
# Dictionnay => set only once per session
...
The argument `style` defines how the table looks. It allows an in-depth customization of the table. The tables is split into several components, each allowing some customization. The components of a table are described by the following figure:

![](https://github.com/lrberge/fixest/blob/master/vignettes/images/etable/style_explanation.png?raw=true)

The argument `style` must be a list, the names of which correspond to the different components. Each elements of this list must be a single character string of the form `"keyword1:value1; keyword2:value; etc"`. The missing components fall back to their default values. The components, with the keywords they accept and their defaults are:
...
Before illustrating how to use the `style` argument, first is shown how to change the defaults.

#### Changing the defaults 

To permanenetly change the defaults, just use the function `setFixest_etable`:
...
And now, using the exact same line of code as in the example, we obtain table with a different style:
