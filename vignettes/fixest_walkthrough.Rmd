...
# the never treated)
res_twfe = feols(y ~ x1 + i(treated, time_to_treatment, ref = c(-1, -1000)) | id + year, base_stagg)
...
coefplot(list(res_twfe, res_sa20), sep = 0.5)
...
This time the aggregation of the coefficients is operated across each cohort for non-negative time to treatment. Note that the estimated ATT compares favourably with the true value of `r with(base_stagg(treatment_effect_true[time_to_treatment >= 0]))`.
