...
etable(gravity_pois, gravity_negbin, gravity_ols,
         vcov = "twoway", subtitles = c("Poisson", "Negative Binomial", "Gaussian"))
```
```{r, echo=FALSE, results='asis'}
tab = etable(gravity_pois, gravity_negbin, gravity_ols, vcov = "twoway", subtitles = c("Poisson", "Negative Binomial", "Gaussian"))
# problem to display the second empty line in markdown
knitr::kable(tab[-2, ])
```

We added the argument `vcov="twoway"` to cluster the standard-errors for all estimations. As can be seen this function gives an overview of the estimates and standard-errors, as well as some quality of fit measures. The argument `subtitles` is used to add information on each estimation column.
...
The version *0.10.0* of `fixest` introduced the argument `vcov`. This argument is highly versatile and single-handedly manages how the standard-errors are computed. This argument can be many things, so bear with me, and now, let's start trying to describe it!
...
So far (version 0.10.0), six types of standard-errors can be computed, here are their keywords and a brief description:
 ...
 - `DK`, `driscoll_kraay`: Driscoll and Kraay (1998) SEs for panel data, assumes cross-sectional correlated and serially correlated errors.
...
```
Of course, if the panel identifier is missing, the automatic clustering falls back to the fixed-effect:
```{r}
# Removing the panel
...
feols(depth ~ mag, quakes, conley(100, distance = "spherical"))
```
...
The value of the SEs can (sometimes quite surprisingly) be impacted by the type of small sample correction (SSC) applied. In `fixest` you can monitor the SSC with the function `ssc()`. Details on how the SSC are computed can be found in the dedicated vignette: [On standard-errors](https://lrberge.github.io/fixest/articles/standard_errors.html) -- here we detail only the implementation. 
...
Finally, we're approaching the end of the tour of the `vcov` argument. That argument can also accept:
 ...
 - a matrix, which will be used as the VCOV (can be useful in `summary` or `etable`)
