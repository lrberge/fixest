...
Say we want to estimate `y` as a function of `x1`, but controlling for `x2`. Moreover, we think that the slope coefficient of our `x2` control variable should be allowed to vary by the `fe1` fixed-effect variable. We just have to do:
...
As we'll see, `i()` is more versatile than just creating interaction terms. It is also able to coerce individual variables to a factor and, compared to base R `factor()`, does so in a way that allows for better synergies with other `fixest` functions like `etable` and `coefplot`. This will allow us to perform some common estimation tasks much more easily.

We now illustrate the main uses of `i()` with the help of some examples.

### A compact syntax for factors and interactions: `i()`

The function `i()` accepts the arguments `ref`, `drop` and `keep`,  which are used to identify which level to keep. Note that the argument `ref` accepts only one value while `drop` accepts several. Here's an example how to use them:
...
Now that we've seen the basic `i()` functionality with a single variable, let's see how it can be used in an interaction term context. We will consider two DiD examples of increasing complexity.
...
Here, `i(treat, period, 5)` creates a set of interact terms: One `treat` dummy for each `period`, but excluding period 5 as a reference. While the explicit reference period is optional, note that we need a reference to avoid a collinearity problem posed by the `id` fixed effects. More to the point, for this DiD analysis we are interested in whether the treatment generated a sustained impact on our outcome variable relative to the control group. This makes period 5 (immediately prior to treatment) the obvious choice to base our relative comparison. It also demonstrates the advantages over the equivalent base `treat*factor(period)` expansion, which will automatically use period 1 as a reference. Of course, we could be specify the levels of `factor(period)` ahead of time. But this is rather tedious, whereas `i()` allows us to do so concisely on the fly.

To display the treatment effects on a graph, we can use the function `coefplot`:

```{r, fig.width=7}
coefplot(est_did)
```

The function `coefplot` detects that the results contain interactions and plots the appropriate graph. In case you wanted to display al the coefficients, including `x1`, you can use the argument `only.inter = FALSE`.

### Staggered difference-in-differences (Sun and Abraham)

*Note that the content of this section is only valid for `fixest` version 0.8.1 or later.*

Despite TWFE's long-standing popularity for DiD designs, recent research has a shone a light on the biases that in can introduce under differential timing. For example, under staggered treatment or policy rollouts. 

For staggered DiD analyzes, `fixest` provides convenience functions for implementing the method of Sun and Abraham (hereafter: SA), which is forthcoming at _Journal of Econometrics_. The summary version is that we again use `i()` to create `cohort x time to treatment` dummies. But we then use the `aggregate()` function to recover the correct time treatment effects as per SA. 
...
Let's walk through an example with generated data, where (a) different cohorts get the treatment at different periods, and (b) the effect of the treatment is cohort-specific and increasing over time. First generate the data:
...
Here's a quick visual depiction of the data. Again, notice the staggered treatment rollout and time-varying intensity.

```{r, echo=FALSE}
library(ggplot2)
ggplot(aggregate(base[, c('year_treated', 'y_true')], 
                 by = list(year = base$year, group = base$group), 
                 mean), 
       aes(year, group, fill = year>=year_treated, alpha = y_true)) + 
...
With our generated data in hand, let's move on to the estimations. We'll implement two DiD models:

  1. A "naive" DiD consisting of a standard TWFE interaction (i.e. treatment × time_to_treatment).
  2. A "cohort" DiD that instead creates cohort interactions (i.e. time_to_treatment × group).
...
For our "cohort" model, we can now apply the SA method of aggregating the coefficients by relative time-to-treatment. We'll use the `aggregate()` function alongside some regexp:

```{r}
# SA method: we aggregate the effects for each period
sa_coefs = aggregate(res_cohort, "(ti.*nt)::(-?[[:digit:]])")
sa_coefs
```

The function `aggregate` aggregates the coefficients whose names match the regular expression `"(ti.*nt)::(-?[[:digit:]]+)"`. The two sets of parentheses are important here since the aggregation will be performed for each unique value in between parentheses. In this case the aggregation will be done for each `time_to_treatment` (i.e. `ti.*nt`) and period (i.e. `-?[[:digit:]]`). 

Note that `fixest` objects can also be aggregated on-the-fly via the complementary "agg" argument of `summary.fixest`. This argument calls `aggregate` behind the scenes and can be passed down from high-order functions like `coefplot` and `etable`, which users may find more convenient. For example:

```{r}
# Same result as `sa_coefs`, using the complementary "agg" argument of summary.fixest
res_sa = summary(res_cohort, agg = "(ti.*nt)::(-?[[:digit:]])")
res_sa
```

Now that we've done the careful work of adjusting for staggered treatment, let's compare our results in plot form. As we can see, the TWFE results suffer from varying degrees of bias due to the staggered rollout. In contrast, the aggregated SA results do a much better job of consistently estimating the true effects:

```{r}
# Plot the naive TWFE results
coefplot(res_twfe, ylim = c(-6, 8), pt.join.par = list(lty = 0))

# Add the SA aggregated results
x = c(-9:-2, 0:8) + .35 # shift for the plot
coefplot(res_sa, x = x, add = TRUE, col = 4, pt.pch = 17)
...
legend("topleft", col = c(1, 4, 2), pch = c(20, 17, 15), 
       legend = c("TWFE", "Sun & Abraham", "Truth"))
```
...
This time the aggregation is operated across each cohort for non-negative time to treatment (i.e. `treatment::[^-]`). Note that the estimated ATT compares favourably with the true value of `r mean(base[base$treat_post == 1, "y_true"])`.
