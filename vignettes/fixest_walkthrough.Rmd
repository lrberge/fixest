...
The function `coefplot` detects that the results contain interactions and plots the appropriate graph. In case you wanted to display all the coefficients, including `x1`, you can use the argument `only.inter = FALSE`.
...
For staggered DiD analyzes, `fixest` provides convenience functions for implementing the unbiased method of [Sun and Abraham (2020)](https://doi.org/10.1016/j.jeconom.2020.09.006) --- hereafter: SA20. The summary version is that we again use `i()` to create `cohort x time to treatment` dummies. But we then use the `aggregate()` function to recover the correct time treatment effects as per SA20. 
...
For our "cohort" model, we can now apply the SA20 method of aggregating the coefficients by relative time-to-treatment. We'll use the `aggregate()` function alongside some regexp:

```{r}
# SA20 method: we aggregate the effects for each period
sa20_coefs = aggregate(res_cohort, "(ti.*nt)::(-?[[:digit:]])")
sa20_coefs
```
...
```{r}
# Same result as `sa20_coefs`, using the complementary "agg" argument of summary.fixest
res_sa20 = summary(res_cohort, agg = "(ti.*nt)::(-?[[:digit:]])")
res_sa20
```

Now that we've done the careful work of adjusting for staggered treatment, let's compare our results in plot form:
...
coefplot(res_sa20, x = x, add = TRUE, col = 4, pt.pch = 17)
...
legend("topleft", col = c(1, 4, 2), pch = c(20, 17, 15), 
       legend = c("TWFE", "Sun & Abraham (2020)", "Truth"))
```
